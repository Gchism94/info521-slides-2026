...
To recap Module 2. Bias-variance: test error is bias squared, plus variance, plus the irreducible sigma-squared — and the U is their sum. Ridge: add lambda-times-the-squared-weights to the loss, and get the closed form X-transpose-X plus lambda-I, inverse, X-transpose y — always invertible, so uniqueness comes for free. And tune lambda by cross-validation, the knob that balances the whole tradeoff. There's no standalone checkpoint for this module; the discipline gets assessed through your modeling choices in Part 1. Carry one thing forward: ridge is a prior in disguise, and Module 4 makes that exact.
